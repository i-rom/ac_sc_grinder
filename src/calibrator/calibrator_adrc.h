...
#define ADRC_SAFETY_SCALE 0.9

#define ADRC_KP_SETPOINT 0.5
#define ADRC_OBSERVERS_SETPOINT 0.5
#define ADRC_OBSERVERS_START_SETPOINT 0.3
...
public:
    ...
    bool tick(io_data_t &io_data) {
        ...
        // T - motor time constant, T = 1/3 * motor_start_time,
        // or 1/3 * (1/2 * motor_start_stop_time)
        //
        // cfg_adrc_b0_inv = 1/b0
        regulator.cfg_adrc_b0_inv = fix16_div(F16(2.0 * 3.0), motor_start_stop_time);

        adrc_param_attempt_value = fix16_mul(F16(MIN_ADRC_KPxB0),
                                             regulator.cfg_adrc_b0_inv);
        ...
        iteration_step = fix16_mul(F16(INIT_KP_ITERATION_STEP),
                                   regulator.cfg_adrc_b0_inv);
        ...
        while (iterations_count < max_iterations)
        {
            ...
            regulator.cfg_adrc_Kp = fix16_mul(F16(MIN_ADRC_KPxB0),
                                              regulator.cfg_adrc_b0_inv);
        ...
        regulator.cfg_adrc_Kobservers = F16(1.0);
        regulator.adrc_update_observers_parameters();
        ...
        iterations_count = 0;
        // Start calibration with minimal
        // ADRC_KOBSERVERS value
        // Motor will work without overshoot
        // with this value.
        iteration_step = INIT_OBSERVERS_ITERATION_STEP;
        ...
        while (iterations_count < max_iterations)
        {
            ...
            while (!speed_tracker.is_stable_or_exceeded())
            {
                regulator.tick(F16(ADRC_OBSERVERS_START_SETPOINT), meter.speed);
                io.setpoint = regulator.out_power;
        ...
        adrc_observers_calibrated_value = fix16_div(adrc_param_attempt_value, F16(ADRC_SAFETY_SCALE));
        ...
        eeprom_float_write(
            CFG_ADRC_B0_ADDR,
            fix16_to_float(regulator.cfg_adrc_b0_inv)
        );
